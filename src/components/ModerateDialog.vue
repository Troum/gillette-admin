                ...
                <v-row>
                    <v-col class="grey lighten-1 pa-0"
                        ...
                        <img @load="imageLoad"
                             ...
                             :src="$store.getters.dialog.status ? `${url}${$store.getters.dialog.data.file}` : ''">
                    </v-col>
                    <v-col cols="5" class="pl-10">
                        <p><span class="subtitle-2">Дата покупки:</span> {{ $store.getters.dialog.data.bought_date }}</p>
                        <p><span class="subtitle-2">Дата регистрации:</span> {{ $store.getters.dialog.data.register_date }}</p>
...
<script>
    export default {
        ...
        methods: {
            ...
            approve() {
                this.form.user_id = this.$store.getters.dialog.data.participant_id;
                this.$root.$emit('confirm-approve', this.form, 'Одобрить регистрацию пользователя?');
            },
            rotate(angle) {
                this.rotation += angle;
            }
